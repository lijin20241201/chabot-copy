...
__date__ = '2020.05.16'
# 这段代码实现了一个高效的敏感词查找和替换工具，基于 Aho-Corasick 算法。该算法通过构建 Trie 树（前缀树） 和 失败指针
# （Failure Pointer） 来实现多模式的高效匹配。
#这是构建 Trie 树 的基本节点类。
class TrieNode():
    def __init__(self):
        self.Index = 0　# 节点的索引。
        ...
        self.End = False # 布尔值，标记这个节点是否为一个完整的敏感词的结尾。
        self.Char = '' # 该节点对应的字符。
        self.Results = [] # 包含该节点对应的敏感词的索引。
        self.m_values = {} # 存储子节点的字典，键是字符的 ASCII 值，值是子节点。
        self.Failure = None # 失败指针，用于 Aho-Corasick 算法。
...
# 用于优化存储，主要是减少内存使用和提升查找效率
class TrieNode2():
    def __init__(self):
        self.End = False # 标记是否为敏感词的结尾。
        self.Results = [] # 存储匹配的敏感词的索引。
        self.m_values = {} # 子节点字典，存储字符与对应节点的映射。
        self.minflag = 0xffff # 记录节点的字符范围，用于加速查找。
        self.maxflag = 0
