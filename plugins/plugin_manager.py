...
class PluginManager: # 插件管理
    def __init__(self):
        ...
        # 键是事件名,值是handers中有当前事件的插件的插件名
        self.listening_plugins = {} 
        # 键是插件名,值是具体的插件(子类)实例
        self.instances = {} 
        self.pconf = {} # 存储插件原名和其对应的优先级等配置
        self.current_plugin_path = None # 当前插件的路径
        # self.loaded 用来存储已加载的插件。这个字典可以用于跟踪哪些插件已经被加载并可以执行。它通常是从 self.plugins 中的插件元数据加载
        # 并实例化出来的
        # self.loaded 可能看起来像这样：
        #         {
        #     'plugin_1': 'loaded',
        #     'plugin_2': 'loaded',
        # }
        self.loaded = {} # 已加载的插件字典。
    ...
    def register(self, name: str, desire_priority: int = 0, **kwargs):
        ...
        def wrapper(plugincls):  # plugincls 是一个单独的插件类，而不是一堆插件。
            ...
            # 这一行从 kwargs 中获取 namecn 参数（如果存在），并将其赋值给插件类的 namecn 属性。如果 namecn 不存在，则使用
            # name 作为插件的中文名。
            plugincls.namecn = kwargs.get("namecn") if kwargs.get("namecn") != None else name
            # 这一行从 kwargs 中获取 hidden 参数（如果存在），并将其赋值给插件类的 hidden 属性。如果没有提供 hidden，则默认
            # 为 False，表示插件不是隐藏的。
            plugincls.hidden = kwargs.get("hidden") if kwargs.get("hidden") != None else False
            # 这一行将插件类的 enabled 属性设置为 True，表示插件是启用的。这可能是插件的默认状态，表明插件已经成功注册并启用。
            ...
            # 将插件按插件名大写做键,插件类做值放入self.plugins字典中
    ...
    def load_config(self):
        ...
        # modified 用于标记配置文件是否已被修改。初始化为 False，表示默认情况下配置文件没有被修改。
        modified = False
        ...
        if os.path.exists("./plugins/plugins.json"):
            ...
            # 这里使用 SortedDict（可能是一个自定义的类或来自 sortedcontainers 库）对插件按优先级排序。
    ...
    def _load_all_config():
        # 指定了全局插件配置文件的路径，plugins/config.json，这是一个统一存放所有插件配置的地方。
    ...
    def scan_plugins(self):
                ...
                if os.path.isfile(main_module_path):
                    ...
                    try:
                        ...
                        if plugin_path in self.loaded: 
                            if plugin_name.upper() != 'GODCMD': # 如果不是主插件
                        ...
                        # 当你导入一个模块时，Python 会自动处理该模块的依赖。也就是说，如果 import_path 对应的模块（如 my_plugin）导入时，它会尝试导入该模块中所依赖的其
                        # 他模块（如 my_plugin.submodule1、my_plugin.submodule2）以及它们之间的依赖关系。
                        else: # 如果插件还没被加载
                            # loaded里设置的是插件路径和其对应的已加载模块
                        ...
                        continue # 退出当次循环,继续加载下个插件模块
        pconf = self.pconf # 获取当前插件管理的配置
        ...
        for name, plugincls in self.plugins.items(): # 遍历所有插件类
            ...
            if rawname not in pconf["plugins"]: # 这种情况是新导入的插件模块 
                ...
                self.plugins._update_heap(name)  # 更新下self.plugins中的插件优先级顺序
    ...
    # 用于激活插件实例的过程，其中涉及到插件的初始化、事件处理器的注册以及异常处理
    def activate_plugins(self): 
        ...
        for name, plugincls in self.plugins.items(): 
            if plugincls.enabled: # 检查插件是否已启用。
                if 'GODCMD' in self.instances and name == 'GODCMD': # 过滤掉'GODCMD'
                    continue
                try:
                ...
                for event in instance.handlers: #  遍历当前插件实例能够处理的所有事件。
                    ...
                    # listening_plugins是个字典,事件-->包含此事件的插件列表
    ...
    def reload_plugin(self, name: str):
        ...
        if name in self.instances:
            ...
            for event in self.listening_plugins:
                # 如果当前插件正在监听此事件，则从监听列表中移除它
                if name in self.listening_plugins[event]:
                    self.listening_plugins[event].remove(name) # 从监听列表中移除当前插件
            # 清空插件实例的handlers字典中的所有项
            self.instances[name].handlers.clear()
            del self.instances[name]  # 从实例字典中删除该插件对应的项
            # 重新激活所有插件，可能是为了加载更新后的插件或恢复默认状态
            self.activate_plugins()
            return True # 返回True表示操作成功
        # 如果插件从未被实例化过，则返回False表示操作失败
        return False
    # 加载插件
    def load_plugins(self):
        self.load_config()  # 从./plugins/plugins.json加载插件优先级之类的配置到pconf
        self.scan_plugins() # 扫描加载所有插件模块,更新pconf或self.plugins
    ...
    def set_plugin_priority(self, name: str, priority: int):
        ...
        if self.plugins[name].priority == priority: # 如果要设置的优先级和当前插件的优先级一致,直接返回True
            return True
        # 其他情况下，改变插件管理字典中插件的优先级
        self.plugins[name].priority = priority 
        self.plugins._update_heap(name) # 更新字典的排序
        ...
        self.refresh_order() # 刷新self.listening_plugins中时间对应的插件的顺序(按插件的优先级)
        return True
    # 启用插件
    def enable_plugin(self, name: str):
        ...
        if not self.plugins[name].enabled: # 如果当前名称的插件未启用
            ...
            self.save_config() # 将更改写到配置文件中
            # 激活插件(包括创建插件实例,绑定插件对相应时间的监听),返回激活失败的插件列表
