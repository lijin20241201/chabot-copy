...
# @singleton 相当于执行了 singleton(WechatChannel)，将 WechatChannel 传入 singleton 函数。
# 在尝试创建 WechatChannel 实例时，get_instance 会被调用，进而确保返回单一实例。
@singleton
class WechatChannel(ChatChannel):
    NOT_SUPPORT_REPLYTYPE = [] # 会覆盖父类中的同名类属性
    # 单例模式确保类只有一个实例（无论调用多少次构造函数，都会返回同一个实例）。
    # 父类的初始化方法会在每次实例化时调用，即使是通过单例模式获得的同一个实例
    # 这就导致了：即使只有一个实例，每次尝试创建实例时，父类的 __init__ 方法会被多次调用，并且每次调用时都启动了一个新的线程。
    def __init__(self):
        super().__init__() 
        # 实例化时，会为每个实例创建一个 ExpiredDict，表示过期消息的存储字典。
        # ExpiredDict 是一个带有过期时间限制的字典，过期时间通过配置获取，默认为3600秒（1小时）。
        # self.receivedMsgs 和 self.auto_login_times 是同一个实例的属性，所有通过单例模式获得的引用都会访问它们的相同数据。
        self.receivedMsgs = ExpiredDict(conf().get("expires_in_seconds", 3600))
        # 初始化 auto_login_times，表示自动登录的次数。默认值是 0。
        # 该属性用于跟踪尝试登录的次数，通常用于限制登录次数等逻辑。
