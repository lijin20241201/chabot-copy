...
import logging
import os
# Python 的一个标准库模块，用于序列化和反序列化 Python 对象。它允许你将复杂的 Python 数据结构（如列表、字典、类实例等）转换为字
# 节流，以便保存到文件或通过网络传输，然后再将其恢复为原来的对象。
import pickle
import ast
import copy
...
config = Config()
def drag_sensitive(config): # 返回加密数据
    try:
...
def load_config():
    ...
    # 通过环境变量来覆盖配置文件中的默认值，特别是敏感信息（如 API 密钥、数据库密码等）。这样做可以确保这些敏感信息不会被硬编码在代码
    # 仓库中，并且可以在不同环境中灵活配置。
    for name, value in os.environ.items():
        ...
        if name in available_setting:
            ...
            try:
                ...
                logger.info(f"Failed to parse value for key '{name}': {value}.")
...
plugin_config = {} # 全局的插件配置
# 把一个配置字典写到全局插件配置里
def write_plugin_config(pconf: dict):
    global plugin_config
    for k in pconf:
        plugin_config[k.lower()] = pconf[k]
# 移除指定名称的插件配置(从配置中删除key-value)
